...
local Eclipse = {}; Eclipse.__index = Eclipse
local objects = (runService:IsStudio() and game:GetService("ReplicatedStorage"):FindFirstChild("Eclipse")) or game:GetObjects("rbxassetid://17799632030")[1]:Clone()
...
-- Light blue color (same as magnet hitbox)
local LIGHT_BLUE = Color3.fromRGB(85, 170, 255)
-- Green color for enabled items
local ENABLED_GREEN = Color3.fromRGB(84, 252, 84) -- #54fc54
...
local Category = {}; Category.__index = Category; do
	local Module = {}; Module.__index = Module; do
		function Module.new(title, root, non, f)
			...
			self.Update = function(a)
				...
				if not non then
					-- Change to green when enabled instead of blue
					self.UI.Module.TextColor3 = self.Value and ENABLED_GREEN or Color3.fromHex("#fff")
				end
	...
	function Category.new(title, icon, root)
		...
		-- Update category frame to use light blue color
		self.UI.BackgroundColor3 = LIGHT_BLUE
...
function Eclipse:Create()
	local self = setmetatable({}, Eclipse)
	self.UI = objects.Bleachhack:Clone()
	self.UI.Name = "Eclipse Hub"
	self.UI.Parent = IS_STUDIO and player.PlayerGui or ((gethui and gethui()) or game:GetService("CoreGui"))
	self.Categories = {}

	-- Update UI elements with light blue color
	self.UI.ModulesList.Bar.BackgroundColor3 = LIGHT_BLUE
	...
	runService.RenderStepped:Connect(function()
		local chroma = LIGHT_BLUE -- Use light blue instead of rainbow color
