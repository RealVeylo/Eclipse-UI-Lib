...
local EclipseHub = {}; EclipseHub.__index = EclipseHub
...
local Category = {}; Category.__index = Category; do
	local Module = {}; Module.__index = Module; do
		function Module.new(title, root, non, f)
			...
			-- Set default tab color to light blue
			self.UI.Module.BackgroundColor3 = Color3.fromRGB(35, 35, 35) -- Dark background
			self.UI.Module.BorderColor3 = Color3.fromRGB(85, 170, 255) -- Light blue border
			...
			moduleTextLabel.TextColor3 = Color3.fromRGB(85, 170, 255) -- Set to light blue
			...
			self.Update = function(a)
				...
				if not non then
					-- Change from blue to green when enabled
					self.UI.Module.TextColor3 = self.Value and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 255, 255)
					-- Add highlight color when enabled
					self.UI.Module.BorderColor3 = Color3.fromRGB(85, 170, 255) -- Always light blue border
				end
		...
		function Module:CreateToggle(data)
			...
			-- Add light blue border
			ui.BorderColor3 = Color3.fromRGB(85, 170, 255)
			...
			data.Update = function()
				ui.TextColor3 = data.Value and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 255, 255)
			end
		...
		function Module:CreateSlider(data)
			...
			ui.Parent = self.UI.Dropdown.Frame
			-- Add light blue border and theme
			ui.BorderColor3 = Color3.fromRGB(85, 170, 255)
			ui.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
			ui.Frame.BackgroundColor3 = Color3.fromRGB(85, 170, 255) -- Light blue slider
	...
	function Category.new(title, icon, root)
		...
		-- Style the category with light blue accents
		self.UI.Title.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
		self.UI.Title.BorderColor3 = Color3.fromRGB(85, 170, 255)
		self.UI.Title.Title.TextColor3 = Color3.fromRGB(255, 255, 255)
		self.UI.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
		self.UI.BorderColor3 = Color3.fromRGB(85, 170, 255)
		self.UI.List.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
		self.UI.List.BorderColor3 = Color3.fromRGB(85, 170, 255)
...
function EclipseHub:Create(options)
    local self = setmetatable({}, EclipseHub)

    -- Set default options
    options = options or {}
    options.MainColor = options.MainColor or Color3.fromRGB(25, 25, 25)
    options.AccentColor = options.AccentColor or Color3.fromRGB(85, 170, 255) -- Light blue accent
    options.OutlineColor = options.OutlineColor or Color3.fromRGB(85, 170, 255) -- Light blue outline
    options.BackgroundColor = options.BackgroundColor or Color3.fromRGB(30, 30, 30)
    options.TextColor = options.TextColor or Color3.fromRGB(255, 255, 255)
    options.Font = options.Font or Enum.Font.GothamBold
    options.Name = options.Name or "Eclipse Hub"
    options.Version = options.Version or "1.0.0"
    options.HidePingFPS = options.HidePingFPS or false

    self.UI = objects.Bleachhack:Clone()
    self.UI.Name = options.Name -- Rename from BleachHack to Eclipse Hub
    self.UI.Parent = IS_STUDIO and player.PlayerGui or ((gethui and gethui()) or game:GetService("CoreGui"))
    self.Categories = {}

    -- Apply custom colors
    self.UI.ModulesList.BackgroundColor3 = options.MainColor
    self.UI.ModulesList.BorderColor3 = options.OutlineColor
    -- Add more Light blue accents
    self.UI.ModulesList.Bar.BackgroundColor3 = options.AccentColor
    self.UI.ModulesList.List.BackgroundColor3 = options.BackgroundColor

    -- Server info panel styling
    self.UI.Server.BackgroundColor3 = options.MainColor
    self.UI.Server.BorderColor3 = options.OutlineColor

    -- Rename title
    self.UI.ModulesList.List.Title.Text = options.Name

    -- Apply custom button appearance if exists
    if self.UI:FindFirstChild("Button") then
        self.UI.Button.BackgroundColor3 = options.MainColor
        self.UI.Button.BorderColor3 = options.OutlineColor
        if self.UI.Button:FindFirstChild("Text") then
            self.UI.Button.Text.Text = options.Name
        end
    end

    -- Hide ping/fps if requested
    if options.HidePingFPS then
        self.UI.Server.Visible = false
    end
    ...
    runService.RenderStepped:Connect(function()
        -- Use light blue color (85, 170, 255) for tab highlights instead of rainbow
        local highlightColor = Color3.fromRGB(85, 170, 255)
        local pingColor = valueToColor(ping * 500, 200)
        local fpsColor = valueToColor2(fps, 60)

        self.UI.ModulesList.Bar.BackgroundColor3 = highlightColor
        ...
        for index, element in pairs(self.UI.ModulesList.List:GetChildren()) do
            ...
            element.TextColor3 = highlightColor
        end
...
function EclipseHub:Import(data)
